...
[L_vec, ntiles] = reduce_to_connected_components(L_all);
L = L_vec(1);
%%
disp('Generating json string ...');
jstr = PM_json(L);
fnpmjson = ...
['/nobackup/flyTEM/khairy/FAFB00v13/point_match_clusters/' ...
'example01_FAFB_' num2str(nfirst) '_' num2str(nlast) '_pm.json'];
str1 = jstr(1:1000000000);
str2 = jstr(1000000001:end);
fid = fopen(fnpmjson,'w');
disp('Writing json string to disk');
fprintf(fid,'%s',str1);
fclose(fid);
fid = fopen(fnpmjson,'a');
disp('Writing json string to disk');
fprintf(fid,'%s',str2);
fclose(fid);
