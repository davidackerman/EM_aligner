...
%% [2] generate montage-scapes and montage-scape point-matches
 precalc_ids = '/nobackup/flyTEM/khairy/FAFB00v13/montage_scape_pms/solver_1to100_01.00_100.00/ids.txt';
 precalc_matches = '/nobackup/flyTEM/khairy/FAFB00v13/montage_scape_pms/solver_1to100_01.00_100.00/matches.txt';
 precalc_path = '/nobackup/flyTEM/khairy/FAFB00v13/montage_scape_pms/solver_1to100_01.00_100.00';
run_now = 0;
...
%mL3 = mLR;
opts.lambda = 1e2;
[mL3, errA] = solve_affine_explicit_region(mLR, opts); % obtain an affine solution
mL3s = split_z(mL3);

%% optional: Render montage scapes
% if run_now==0
%     for ix = 1:numel(mLR.tiles)
%         fn_im = sprintf('%s/layer_images/%.1f.png', precalc_path,ix);
%         mLR.tiles(ix).path = fn_im;
%     end
% end
% mLR = update_XY(mLR);
% mLR = get_bounding_box(mLR);
% Wbox = [mLR.box(1) mLR.box(3) mLR.box(2)-mLR.box(1) mLR.box(4)-mLR.box(3)];
% disp(Wbox);
% dir_out = ['/tier2/flyTEM/khairy/render_experiments/render_rough'  num2str(nfirst) '_' num2str(nlast)];
% kk_mkdir(dir_out);
% disp(dir_out);
% kk_clock();
% disp('Rendering ...');tic;
% [im, Wbox, imlabel, M] = render_poly_07(mLR.tiles, 1.0, Wbox, 0,[], dir_out);
% disp('Done!');toc;
...
for lix = 1:numel(L_montage)
    ...
    tiles = L3(lix).tiles;
    parfor tix = 1:numel(L3(lix).tiles)
        newT = tiles(tix).tform.T * tmx1 * smx * mL3s(lix).tiles(1).tform.T * tmx2 * (invsmx);
