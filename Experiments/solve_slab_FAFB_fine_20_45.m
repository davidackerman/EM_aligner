...
% configure source collection
rcsource.stack          = 'EXP_dmesh_rough_P1_20_45';
rcsource.owner          ='flyTEM';
...
rctarget_align.stack          = ['EXP_dmesh_fine_P1_' num2str(nfirst) '_' num2str(nlast)];
...
disp(err1/numel(mLra.tiles));
disp('Error in percent pixels per point-match:');
disp(err1/size(A,1) * 100);


[mLra, Af, Sf] = filter_based_on_tile_area(mLra, opts.outlier_lambda);
