...
if options.store_residual_outlier_pair_information
    for rc_index=1:numel(rcs) %output file for each rc
        ...
        fprintf(summary_fid,'%s\n',cell2mat({'Z,', 'Max PM Residual,', 'Tile 1,', 'Tile 2,', 'PM X,', 'PM Y'}));
        fclose(summary_fid);
        ...
        for z = zu
            ...
            if exist(file_name, 'file') == 2 %otherwise there were no outliers for that section
                [string_z, string_residual, string_tile_1, string_tile_2, string_pm_x, string_pm_y] = textread(file_name, '%s %s %s %s %s %s');
                output_data = [string_z, string_residual, string_tile_1, string_tile_2, string_pm_x, string_pm_y];
